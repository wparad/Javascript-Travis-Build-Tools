...
function Travis() {
	this.Version = GetVersion(process.env.TRAVIS_PULL_REQUEST, process.env.TRAVIS_BRANCH, process.env.TRAVIS_BUILD_NUMBER);
}
...
function GetVersion(pull_request, branch, build_number)
{
	...
	//Builds of release branches (or locally or on server)
		...
		release_version = branch.match(/^release[\/-](\d+\.\d+)$/i)[1];
	}

	return release_version + '.' + (build_number == undefined ? '0' : build_number);;
}
