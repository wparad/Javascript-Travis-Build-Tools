...
function Travis(ignore_travis_build_environment) {
	this.pull_request = ignore_travis_build_environment ? undefined : process.env.TRAVIS_PULL_REQUEST;
	this.branch = ignore_travis_build_environment ? undefined : process.env.TRAVIS_BRANCH;
	this.build_number = ignore_travis_build_environment ? undefined : process.env.TRAVIS_BUILD_NUMBER;
...
function GetVersion(pull_request, branch, build_number)
{
	...
	return (release_version + '.' + (build_number == undefined ? '0' : build_number) + '.0.0.0.0').split('.').slice(0, 3).join('.');
}

function GetPullRequest(travis_pull_request){
	var travis_pull_request = travis_pull_request || this.travis_pull_request;
